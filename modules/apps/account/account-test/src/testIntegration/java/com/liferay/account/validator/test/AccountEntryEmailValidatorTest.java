...
public class AccountEntryEmailValidatorTest {
		...
		try {
			Configuration[] configurations =
				_configurationAdmin.listConfigurations(
					String.format(
						"(&(companyId=%s)(factoryPid=%s))", _companyId,
						_FACTORY_PID));

			if (configurations != null) {
				configuration = configurations[0];

				oldProperties = configuration.getProperties();
			}
			...
			ConfigurationTestUtil.saveConfiguration(
				...
				HashMapDictionaryBuilder.<String, Object>put(
					"blockedEmailDomains",
					StringUtil.merge(
						validatorArgs.blockedDomains, StringPool.NEW_LINE)
				).put(
	...
	private static final String _FACTORY_PID =
		AccountEntryEmailDomainsConfiguration.class.getName() + ".scoped";
